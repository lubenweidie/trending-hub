...
- `publish()` → `_editor_opened` 为 True 时跳过 `open_editor()`，先调 `_enrich_article()`（源提取+AI扩写+配图），再执行内容填充+封面+发布
...
**数据分析验证的有效策略：**
- 标题：身份认同钩子（点名具体群体）> 利他性 > 纯新闻陈述
- 内容：新华网/人民日报权威来源 + 老百姓视角解读 = 最高阅读量
- 百家号流量远大于头条，是主力平台
- 政策解读类在百家号表现最好（最高 1586 阅读）
- 奇闻趣事/娱乐八卦有稳定流量（19/13/24 阅读），适合内容矩阵多元化
...
- **知乎 collector 偶发超时** — `opencli browser fetch` 15 秒超时，高峰期概率较高
- **`execCommand('insertText')` 已修复** — 百家号 `fill_content` 追加模式改用 `insertAdjacentHTML`，Lexical 编辑器不会拦截 DOM API
- **`_real_click()` DPI 坐标偏差已修复** — 改用 `SetProcessDpiAwareness(2)` + `SendInput` 绝对坐标
